...
def train_balance(model, train_dataloader, val_dataloader, optimizer, criterion, epoch, args, device, filename):
    ...
    for i, (batch_features, batch_labels) in enumerate(train_dataloader):
        ...
        all_ce_loss_list = []
        # TODO: 对每个标签都要做不同的处理，每个标签都使用不同的模型，然后计算梯度，最后累加梯度进行更新
        for column in range(len(support_labels[0])):
            ...
            # val_dataloader,是一个list，里面是8个DataLoader，对应着痛风与八种疾病共病的数据
            data_loader = val_dataloader[column]
            for batch in data_loader:
                val_features, val_labels = batch
                # 在这里处理数据，可以将其保存到变量中或进行其他操作
